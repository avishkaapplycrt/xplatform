...
<style>
  ...
  .step-card:hover {
    border-color: #95bf47;
    box-shadow: 0 4px 16px #95bf4714;
  }
  .step-number {
    width: 36px;
    height: 36px;
...
<header class="flex-shrink-0 bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between">
  <div class="flex items-center gap-3">
    ...
    <div class="flex items-center gap-2">
      ...
      <div>
        <h1 class="text-[16px] font-semibold text-gray-900">Connect Shopify</h1>
...
<div class="flex-1 overflow-y-auto px-5 py-4">
  ...
  <div id="step-1" class="step-card mb-4">
    <div class="flex items-start gap-4">
      ...
      <div class="flex-1">
        <h3 class="text-[14px] font-bold text-gray-900 mb-1">Enter Your Shopify Site Details</h3>
        <p class="text-[12px] text-gray-500 mb-4">Provide your Shopify store URL and install our app for e-commerce tracking.</p>

        <form id="connectForm" onsubmit="submitConnection(event)">
          <input type="hidden" id="modalPlatform" name="platform" value="shopify">
          ...
          <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <p class="form-hint">The full URL of your Shopify site</p>
            </div>
            <div>
              <label class="form-label">Site Name <span class="text-gray-400 font-normal">(optional)</span></label>
              <input type="text" name="site_name" class="form-input" placeholder="My Shopify Site"
          ...
          <div class="mb-4">
            ...
            <div class="flex gap-2 mb-3">
              <button type="button" class="method-tab active" onclick="switchMethod('app')">Shopify App Store</button>
              <button type="button" class="method-tab" onclick="switchMethod('manual')">Theme Code</button>
            </div>

            <div id="method-app" class="method-panel active">
              <div class="info-box mb-3">
                <p class="info-box-title">Shopify App Store</p>
                <p class="info-box-text">Install our official app from the Shopify App Store for seamless integration.</p>
              </div>
              <div class="mt-3">
              <div class="install-step"><div class="install-step-num">1</div><div class="install-step-text">Go to your Shopify Admin → Apps → App Store</div></div>
              <div class="install-step"><div class="install-step-num">2</div><div class="install-step-text">Search for "Scorementor Analytics"</div></div>
              <div class="install-step"><div class="install-step-num">3</div><div class="install-step-text">Click Install and approve permissions</div></div>
              <div class="install-step"><div class="install-step-num">4</div><div class="install-step-text">The app will auto-configure tracking for your store</div></div>
            </div>
            </div>

            <div id="method-manual" class="method-panel">
              <div class="info-box mb-3">
                <p class="info-box-title">Theme Code</p>
                <p class="info-box-text">Manually add the tracking code to your Shopify theme files.</p>
              </div>
              <div class="mt-3">
              <div class="install-step"><div class="install-step-num">1</div><div class="install-step-text">Go to Online Store → Themes → Edit Code</div></div>
              <div class="install-step"><div class="install-step-num">2</div><div class="install-step-text">Open theme.liquid in the Layout folder</div></div>
              <div class="install-step"><div class="install-step-num">3</div><div class="install-step-text">Paste the tracking code before the closing &lt;/head&gt; tag</div></div>
              <div class="install-step"><div class="install-step-num">4</div><div class="install-step-text">Save the file and clear cache if needed</div></div>
            </div>
            </div>
          </div>
  ...
  <div id="step-2" class="step-card mb-4">
    <div class="flex items-start gap-4">
      ...
      <div class="flex-1">
        ...
        <p class="text-[12px] text-gray-500 mb-4">Add this tracking code to your Shopify site to start collecting events.</p>

        <div id="install-app" class="method-panel active">
          <div class="info-box mb-3">
            <p class="info-box-title">Shopify App Store Installation</p>
  ...
  <div id="step-3" class="step-card mb-4">
    <div class="flex items-start gap-4">
      ...
      <div class="flex-1">
        ...
        <p class="text-[12px] text-gray-500 mb-4">Your Shopify site is now connected and tracking events.</p>
...
function forceVerifyConnection() {
  const btn = document.getElementById('forceVerifyBtn');
  const status = document.getElementById('verifyStatus');
  ...
  status.style.display = 'inline-flex';
  status.className = 'verify-badge pending';
  status.innerHTML = '<span class="w-1.5 h-1.5 rounded-full bg-current inline-block"></span> Forcing verify...';

  fetch('{{ url("/app/website-connections") }}' + '/' + currentConnectionId + '/verify?force=true')
    .then(r => r.json())
    .then(res => {
      if (res.success && res.healthy) {
        status.className = 'verify-badge success';
        status.innerHTML = '<span class="w-1.5 h-1.5 rounded-full bg-current inline-block"></span> Manually Verified!';
        updateProgress(3);
        btn.style.display = 'none';
      } else {
        status.className = 'verify-badge error';
        status.innerHTML = '<span class="w-1.5 h-1.5 rounded-full bg-current inline-block"></span> Force verify failed';
      }
    })
    .catch(err => {
      console.error(err);
...
function disconnectSite() {
  if (!currentConnectionId) return;
  if (!confirm('Are you sure you want to disconnect this Shopify site? All tracking will stop.')) return;
  ...
  .then(res => {
    if (res.success) {
      alert('Shopify site disconnected successfully.');
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Advance to step 2 (enable tracking code section)
  updateProgress(2);

  // If already verified, advance to step 3
...
// Initialize step states on page load
(function() {
  const step2 = document.getElementById('step-2');
  const step3 = document.getElementById('step-3');
  if (step2) step2.classList.add('step-disabled');
  ...
  // Always enable verify button if connection exists on page load
