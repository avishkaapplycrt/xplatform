...
@section('title', 'CRM Analytics - Activities')
...
<div class="flex flex-col h-full overflow-hidden bg-white">
  ...
  <header class="flex-shrink-0 bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-[16px] font-semibold text-gray-900">Activity Tracking</h1>
        <p class="text-[11px] text-gray-500 mt-0.5">Monitor team activities and follow-ups</p>
      </div>
    </div>
    <div class="flex items-center gap-3">
  ...
  <div class="flex-1 overflow-y-auto px-5 py-5">
    ...
    <div class="flex items-center gap-1 mb-5 border-b border-gray-200 pb-0">
      ...
      <a href="{{ route('client.reports.crm.activities') }}" class="px-4 py-2.5 text-[12px] font-medium rounded-t-lg transition text-blue-600 border-b-2 border-blue-600 bg-blue-50/50" style="text-decoration: none;">
        Activities
      </a>
    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
      <div class="px-4 py-3 border-b border-gray-100">
        ...
        <p class="text-[11px] text-gray-400 mt-0.5">Contact and deal updates pulled in from your connected CRM.</p>
      </div>
      ...
      <div class="divide-y divide-gray-50">
        ...
        <div class="px-4 py-3 flex items-center gap-3">
          <span class="w-2 h-2 rounded-full flex-shrink-0 {{ $event->type === 'deal' ? 'bg-cyan-500' : 'bg-violet-500' }}"></span>
          <div class="flex-1 min-w-0">
            <p class="text-[13px] text-gray-800 truncate">{{ $event->label }}</p>
            @if($event->meta)
            <p class="text-[11px] text-gray-500 mt-0.5">{{ $event->meta }}</p>
            @endif
          </div>
          <span class="text-[11px] text-gray-400 flex-shrink-0">{{ \Carbon\Carbon::parse($event->timestamp)->diffForHumans() }}</span>
        </div>
        @endforeach
      </div>
      @endif
