...
@section('title', 'CRM Analytics - Pipeline')
...
<div class="flex flex-col h-full overflow-hidden bg-white">
  ...
  <header class="flex-shrink-0 bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-[16px] font-semibold text-gray-900">Sales Pipeline</h1>
        <p class="text-[11px] text-gray-500 mt-0.5">Visualize your deal pipeline</p>
      </div>
    </div>
    <div class="flex items-center gap-3">
  ...
  <div class="flex-1 overflow-y-auto px-5 py-5">
    ...
    <div class="flex items-center gap-1 mb-5 border-b border-gray-200 pb-0">
      ...
      <a href="{{ route('client.reports.crm.pipeline') }}" class="px-4 py-2.5 text-[12px] font-medium rounded-t-lg transition text-blue-600 border-b-2 border-blue-600 bg-blue-50/50" style="text-decoration: none;">
        Pipeline
      </a>
      ...
      <div class="p-4 space-y-3">
        ...
        <div>
          <div class="flex items-center justify-between mb-1">
            <span class="text-[12.5px] font-medium text-gray-700">{{ $stage->stage }}</span>
            <span class="text-[12px] text-gray-500">{{ $stage->deal_count }} deals &middot; ${{ number_format($stage->total_value) }}</span>
          </div>
          <div class="h-2 rounded-full bg-gray-100 overflow-hidden">
            <div class="h-full rounded-full bg-cyan-500" style="width: {{ max(3, round($stage->total_value / $maxValue * 100)) }}%"></div>
          </div>
        </div>
        @endforeach
      </div>
