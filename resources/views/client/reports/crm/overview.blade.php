...
@section('title', 'CRM Analytics - Overview')
...
<div class="flex flex-col h-full overflow-hidden bg-white">
  ...
  <header class="flex-shrink-0 bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-[16px] font-semibold text-gray-900">CRM Analytics</h1>
        <p class="text-[11px] text-gray-500 mt-0.5">Track contacts, deals, and pipeline</p>
      </div>
    </div>
    <div class="flex items-center gap-3">
  ...
  <div class="flex-1 overflow-y-auto px-5 py-5">
    ...
    <div class="flex items-center gap-1 mb-5 border-b border-gray-200 pb-0">
      <a href="{{ route('client.reports.crm.overview') }}" class="px-4 py-2.5 text-[12px] font-medium rounded-t-lg transition text-blue-600 border-b-2 border-blue-600 bg-blue-50/50" style="text-decoration: none;">
        Overview
      </a>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
      <div class="bg-white border border-gray-200 rounded-xl p-4">
        <p class="text-[11px] text-gray-500 font-medium mb-1">Total Contacts</p>
        <p class="text-[20px] font-bold text-gray-900">{{ number_format($data['total_contacts'] ?? 0) }}</p>
      </div>
      <div class="bg-white border border-gray-200 rounded-xl p-4">
        <p class="text-[11px] text-gray-500 font-medium mb-1">Total Deals</p>
        <p class="text-[20px] font-bold text-gray-900">{{ number_format($data['total_deals'] ?? 0) }}</p>
      </div>
      <div class="bg-white border border-gray-200 rounded-xl p-4">
        <p class="text-[11px] text-gray-500 font-medium mb-1">Pipeline Value</p>
        <p class="text-[20px] font-bold text-gray-900">${{ number_format($data['pipeline_value'] ?? 0) }}</p>
      </div>
      <div class="bg-white border border-gray-200 rounded-xl p-4">
        <p class="text-[11px] text-gray-500 font-medium mb-1">Win Rate</p>
        <p class="text-[20px] font-bold text-gray-900">{{ $data['win_rate'] ?? 0 }}%</p>
      </div>
    </div>
