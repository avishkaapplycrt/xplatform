...
@section('title', 'Transaction Analytics - Customers')
...
<div class="flex flex-col h-full overflow-hidden bg-white">
  ...
  <header class="flex-shrink-0 bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-[16px] font-semibold text-gray-900">Customers</h1>
        <p class="text-[11px] text-gray-500 mt-0.5">Your real customer records, synced from Stripe</p>
      </div>
    </div>
    <div class="flex items-center gap-3">
  ...
  <div class="flex-1 overflow-y-auto px-5 py-5">
    ...
    <div class="flex items-center gap-1 mb-5 border-b border-gray-200 pb-0">
      <a href="{{ route('client.reports.transactions.overview') }}" class="px-4 py-2.5 text-[12px] font-medium rounded-t-lg transition text-gray-500 hover:text-gray-700 hover:bg-gray-50" style="text-decoration: none;">
        Overview
      </a>
      ...
      <a href="{{ route('client.reports.transactions.customers') }}" class="px-4 py-2.5 text-[12px] font-medium rounded-t-lg transition text-blue-600 border-b-2 border-blue-600 bg-blue-50/50" style="text-decoration: none;">
        Customers
      </a>
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-4 mb-5" style="max-width:260px">
      <p class="text-[11px] text-gray-500 font-semibold uppercase tracking-wide mb-1">Total Customers</p>
      <p class="text-[22px] font-bold text-gray-900">{{ number_format($data['total']) }}</p>
    </div>
      ...
      <table class="w-full text-[12px]">
        <thead>
          <tr class="bg-gray-50 border-b border-gray-200">
            <th class="text-left px-4 py-2.5 font-semibold text-gray-500 uppercase text-[10px] tracking-wide">Name</th>
            <th class="text-left px-4 py-2.5 font-semibold text-gray-500 uppercase text-[10px] tracking-wide">Email</th>
            <th class="text-left px-4 py-2.5 font-semibold text-gray-500 uppercase text-[10px] tracking-wide">Phone</th>
            <th class="text-right px-4 py-2.5 font-semibold text-gray-500 uppercase text-[10px] tracking-wide">Orders</th>
            <th class="text-left px-4 py-2.5 font-semibold text-gray-500 uppercase text-[10px] tracking-wide">Customer Since</th>
            <th class="text-right px-4 py-2.5 font-semibold text-gray-500 uppercase text-[10px] tracking-wide">Lifetime Value</th>
          </tr>
        </thead>
        <tbody>
          @forelse($data['customers'] as $c)
          <tr class="border-b border-gray-100 last:border-0">
            <td class="px-4 py-2.5 text-gray-900 font-medium">{{ $c['name'] }}</td>
            <td class="px-4 py-2.5 text-gray-700">{{ $c['email'] ?: '—' }}</td>
            <td class="px-4 py-2.5 text-gray-700">{{ $c['phone'] ?: '—' }}</td>
            <td class="px-4 py-2.5 text-right text-gray-700">{{ $c['orders'] }}</td>
            <td class="px-4 py-2.5 text-gray-700">{{ \Carbon\Carbon::parse($c['since'])->format('M j, Y') }}</td>
            <td class="px-4 py-2.5 text-right font-semibold text-gray-900">${{ number_format($c['lifetime_value'], 2) }}</td>
          </tr>
          @empty
          <tr><td colspan="6" class="px-4 py-6 text-center text-gray-400">No customers synced from Stripe yet — hit "Sync Now" on the connection page.</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
