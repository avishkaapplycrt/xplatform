...
@section('title', 'Transaction Analytics - Customer LTV')
...
<div class="flex flex-col h-full overflow-hidden bg-white">
  ...
  <header class="flex-shrink-0 bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-[16px] font-semibold text-gray-900">Customer Lifetime Value</h1>
        <p class="text-[11px] text-gray-500 mt-0.5">Total spend per customer, all-time</p>
      </div>
    </div>
    <div class="flex items-center gap-3">
  ...
  <div class="flex-1 overflow-y-auto px-5 py-5">
    ...
    <div class="grid grid-cols-2 gap-4 mb-5">
      <div class="bg-white border border-gray-200 rounded-xl p-4">
        <p class="text-[11px] text-gray-500 font-semibold uppercase tracking-wide mb-1">Customers (with a completed order)</p>
        <p class="text-[22px] font-bold text-gray-900">{{ number_format($data['customer_count']) }}</p>
      </div>
      <div class="bg-white border border-gray-200 rounded-xl p-4">
        <p class="text-[11px] text-gray-500 font-semibold uppercase tracking-wide mb-1">Average Lifetime Value</p>
        <p class="text-[22px] font-bold text-gray-900">${{ number_format($data['avg_ltv'], 2) }}</p>
      </div>
    </div>
      ...
      <table class="w-full text-[12px]">
        <thead>
          <tr class="bg-gray-50 border-b border-gray-200">
            ...
            <th class="text-right px-4 py-2.5 font-semibold text-gray-500 uppercase text-[10px] tracking-wide">Avg Order</th>
            <th class="text-left px-4 py-2.5 font-semibold text-gray-500 uppercase text-[10px] tracking-wide">Last Purchase</th>
        ...
        <tbody>
          ...
          <tr class="border-b border-gray-100 last:border-0">
            ...
            <td class="px-4 py-2.5 text-right text-gray-700">${{ number_format($c['avg_order'], 2) }}</td>
            <td class="px-4 py-2.5 text-gray-700">{{ \Carbon\Carbon::parse($c['last_seen'])->format('M j, Y') }}</td>
            <td class="px-4 py-2.5 text-right font-semibold text-gray-900">${{ number_format($c['ltv'], 2) }}</td>
          </tr>
          @empty
          <tr><td colspan="5" class="px-4 py-6 text-center text-gray-400">No completed orders with a known customer email yet.</td></tr>
          @endforelse
        </tbody>
      </table>
    </div>
