...
@section('title', 'Chat & Support Analytics - Overview')
...
<div class="flex flex-col h-full overflow-hidden bg-white">
  ...
  <header class="flex-shrink-0 bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-[16px] font-semibold text-gray-900">Chat & Support Analytics</h1>
        <p class="text-[11px] text-gray-500 mt-0.5">Track conversations, response times, and satisfaction</p>
      </div>
    </div>
    <div class="flex items-center gap-3">
  ...
  <div class="flex-1 overflow-y-auto px-5 py-5">
    ...
    <div class="flex items-center gap-1 mb-5 border-b border-gray-200 pb-0">
      <a href="{{ route('client.reports.chat-support.overview') }}" class="px-4 py-2.5 text-[12px] font-medium rounded-t-lg transition text-blue-600 border-b-2 border-blue-600 bg-blue-50/50" style="text-decoration: none;">
        Overview
      </a>
      <a href="{{ route('client.reports.chat-support.conversations') }}" class="px-4 py-2.5 text-[12px] font-medium rounded-t-lg transition text-gray-500 hover:text-gray-700 hover:bg-gray-50" style="text-decoration: none;">
        Conversations
      </a>
      <a href="{{ route('client.reports.chat-support.response-time') }}" class="px-4 py-2.5 text-[12px] font-medium rounded-t-lg transition text-gray-500 hover:text-gray-700 hover:bg-gray-50" style="text-decoration: none;">
        Response Time
      </a>
      <a href="{{ route('client.reports.chat-support.satisfaction') }}" class="px-4 py-2.5 text-[12px] font-medium rounded-t-lg transition text-gray-500 hover:text-gray-700 hover:bg-gray-50" style="text-decoration: none;">
        Satisfaction
      </a>
      <a href="{{ route('client.reports.chat-support.team-performance') }}" class="px-4 py-2.5 text-[12px] font-medium rounded-t-lg transition text-gray-500 hover:text-gray-700 hover:bg-gray-50" style="text-decoration: none;">
        Team
      </a>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
      <div class="bg-white border border-gray-200 rounded-xl p-4">
        <p class="text-[11px] text-gray-500 font-medium mb-1">Conversations</p>
        <p class="text-[20px] font-bold text-gray-900">{{ number_format($data['total_conversations'] ?? 0) }}</p>
      </div>
      <div class="bg-white border border-gray-200 rounded-xl p-4">
        <p class="text-[11px] text-gray-500 font-medium mb-1">Avg Response</p>
        <p class="text-[20px] font-bold text-gray-900">{{ $data['avg_response_time'] ?? '0m' }}</p>
      </div>
      <div class="bg-white border border-gray-200 rounded-xl p-4">
        <p class="text-[11px] text-gray-500 font-medium mb-1">CSAT Score</p>
        <p class="text-[20px] font-bold text-gray-900">{{ $data['csat_score'] ?? 0 }}/5</p>
      </div>
      <div class="bg-white border border-gray-200 rounded-xl p-4">
        <p class="text-[11px] text-gray-500 font-medium mb-1">Open Tickets</p>
        <p class="text-[20px] font-bold text-gray-900">{{ number_format($data['open_tickets'] ?? 0) }}</p>
      </div>
    </div>
    @else
    <div class="bg-white border border-gray-200 rounded-xl p-8 text-center">
      ...
      <h3 class="text-[14px] font-semibold text-gray-800 mb-1">No Chat & Support data yet</h3>
      <p class="text-[12px] text-gray-500 mb-4">Connect your Chat & Support account to start tracking analytics.</p>
      <a href="{{ route('client.chat-support-connections') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-red-600 text-white text-[12px] font-medium rounded-lg hover:bg-red-700 transition" style="text-decoration: none;">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
