...
@section('title', 'Growth Intelligence - Recommendations')
...
<div class="flex flex-col h-full overflow-hidden bg-white">
  ...
  <header class="flex-shrink-0 bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-[16px] font-semibold text-gray-900">AI Recommendations</h1>
        <p class="text-[11px] text-gray-500 mt-0.5">Actionable insights to grow your business</p>
      </div>
    </div>
    <div class="flex items-center gap-3">
  ...
  <div class="flex-1 overflow-y-auto px-5 py-5">
    ...
    <div class="flex items-center gap-1 mb-5 border-b border-gray-200 pb-0">
      <a href="{{ route('client.reports.growth.business-health') }}" class="px-4 py-2.5 text-[12px] font-medium rounded-t-lg transition text-gray-500 hover:text-gray-700 hover:bg-gray-50" style="text-decoration: none;">
        Health Score
      </a>
      ...
      <a href="{{ route('client.reports.growth.recommendations') }}" class="px-4 py-2.5 text-[12px] font-medium rounded-t-lg transition text-blue-600 border-b-2 border-blue-600 bg-blue-50/50" style="text-decoration: none;">
        Recommendations
      </a>
    ...
    <div class="space-y-4">
      ...
      <div class="bg-white border border-gray-200 rounded-xl p-4 flex items-start gap-3">
        <span class="flex-shrink-0 px-2 py-0.5 rounded text-[10px] font-bold uppercase
        ...
        <div>
          <h4 class="text-[13px] font-semibold text-gray-800">{{ $rec['title'] }}</h4>
          <p class="text-[12px] text-gray-500 mt-1">{{ $rec['message'] }}</p>
        </div>
      </div>
      @empty
      <div class="bg-white border border-gray-200 rounded-xl p-8 text-center">
        @if($data['has_data'])
          <p class="text-[14px] text-gray-600 mb-3">No recommendations yet — generate your health report first.</p>
          <a href="{{ route('client.reports.growth.business-health') }}" class="btn-primary" style="text-decoration:none;">Go to Health Score</a>
        @else
          <p class="text-[14px] text-gray-600">Connect a data source to start receiving recommendations.</p>
        @endif
      </div>
      @endforelse
    </div>
