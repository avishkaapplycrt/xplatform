...
<div class="flex flex-col h-full overflow-hidden bg-white">
  ...
  <header class="flex-shrink-0 bg-white border-b border-gray-200 px-6 py-3 flex items-center justify-between">
    <div class="flex items-center gap-3">
      ...
      <div>
        <h1 class="text-[16px] font-semibold text-gray-900">Create Custom Report</h1>
        <p class="text-[11px] text-gray-500 mt-0.5">Select metrics and dimensions to build your report</p>
      </div>
    </div>
    <div class="flex items-center gap-3">
  ...
  <div class="flex-1 overflow-y-auto px-5 py-5">
    ...
    <form action="{{ route('client.reports.custom.store') }}" method="POST" class="max-w-3xl">
      @csrf
      <div class="bg-white border border-gray-200 rounded-xl p-6 mb-4">
        <div class="mb-4">
          ...
          <input type="text" name="name" class="form-input w-full" placeholder="e.g., Monthly Revenue Summary" required>
        </div>
        <div class="mb-4">
          <label class="block text-[12px] font-semibold text-gray-700 mb-2">Description</label>
          <textarea name="description" class="form-input w-full" rows="2" placeholder="What is this report for?"></textarea>
        </div>
      ...
      <div class="flex items-center gap-3">
        <button type="submit" class="px-4 py-2.5 bg-blue-600 text-white text-[12px] font-medium rounded-lg hover:bg-blue-700 transition">Create Report</button>
