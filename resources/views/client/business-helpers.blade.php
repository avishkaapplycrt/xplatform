...
function marketingDbAnswer(stepKey, slug, name, account, c){
  ...
  switch (slug) {
    ...
    case 'forecast_roi_by_audience':
      return head + '<p><strong>Accounts by audience:</strong> Win-back '+winback.length+' (avg trust '+avgScore(winback,'trust')+') · Onboarding '+onboarding.length+' (avg readiness '+avgScore(onboarding,'buying_readiness')+') · Referral '+referral.length+' (avg loyalty '+avgScore(referral,'loyalty')+'). Win-back and onboarding carry the most near-term upside right now.</p>';
...
function renderPerformance(){
  ...
  return '<div class="mg-grid">'+
    '<div class="mg-cell"><div class="mg-h">AVERAGE READINESS IN ACTIVE PLAYS</div>'+
    '<div class="mg-kpi">'+avgScore(active,'buying_readiness')+' <small>across '+active.length+' accounts</small></div>'+
    '<div style="font-size:11.5px;color:var(--g2);margin-top:8px;line-height:1.6">Average buying-readiness across accounts in an active campaign play — the honest signal of near-term potential.</div></div>'+
